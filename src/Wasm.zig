            ...
            switch (kind) {
                ...
                .code => {
                    ...
                    while (n > 0) : (n -= 1) {
                        ...
                        var codeRelocs = std.ArrayListUnmanaged(IR.Linking.Reloc.Entry){};
                        var codeTypes = std.ArrayListUnmanaged(IR.Func.Sig){};
                        defer {
                            codeRelocs.deinit(allocator);
                            codeTypes.deinit(allocator);
                        }
                        var depth: usize = 1;
                        while (depth > 0) {
                            const cur = try self.op(&depth);
                            switch (cur.arg) {
                                .blocktype => |bt| switch (bt) {
                                    //FIXME: not llvm relocatable but must be in our IR model
                                    .idx => unreachable,
                                    else => {},
                                },
                                .xy => |xy| {
                                    if (xy[0] >= types.items.len) return error.NotFound;
                                    //MAYBE: dedupe
                                    try codeRelocs.append(allocator, .{
                                        .type = .typeIndexLeb,
                                        .offset = @truncate(u32, cur.argOffset - codeStart),
                                        .index = @truncate(u32, codeTypes.items.len),
                                    });
                                    try codeTypes.append(allocator, types.items[xy[0]]);
                                },
                                else => {},
                            }
                        }
                        ...
                        f.body = .{
                            .code = .{
                                .bytes = try m_allocator.dupe(u8, self.it.buffer[codeStart..codeEnd]),
                                .types = try m_allocator.dupe(IR.Func.Sig, codeTypes.items),
                                .relocs = try m_allocator.dupe(IR.Linking.Reloc.Entry, codeRelocs.items),
                            },
                        };
                    }
                },
